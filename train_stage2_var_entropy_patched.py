...
from infinity.dataset.build_stage2_dataset import build_stage2_dataloader, move_text_cond_tuple_to_device
...
def build_everything_from_args(args: arg_util.Args, saver):
    ...
    if hasattr(ld_train.dataset, 'h_div_w_template2generator'):
...
def build_model_optimizer(args, vae_ckpt):
    ...
    try:
        ...
        setattr(torch.nn.Linear, 'reset_parameters', linear_reset_parameters)
        setattr(torch.nn.LayerNorm, 'reset_parameters', layernorm_reset_parameters)
    del vae_ckpt
    if args.tini < 0:
    ...
    use_proc_text = getattr(args, 'dataset_backend', 'streaming') in {'proc', 'proc_memmap', 'memmap'}
    if args.online_t5 and not use_proc_text:
    ...
    student_wo_ddp = student_ddp = student_optim = None
    if getattr(args, 'enable_student_entropy', 0):
        if not args.use_bit_label:
            raise NotImplementedError('Tiny entropy student currently requires args.use_bit_label=1.')
        ...
        if args.zero:
            ...
            student_optim = torch.optim.AdamW(student_ddp.parameters(), lr=args.student_lr, weight_decay=args.student_wd)
            print(f'[student] enabled with FSDP, hidden={args.student_hidden_dim}, depth={args.student_depth}, lr={args.student_lr}, start_step={args.student_start_step}')
        else:
            ddp_class = DDP if dist.initialized() else misc.NullDDP
...
def build_dataloaders(args):
    ...
    if dataset_backend in {'proc', 'proc_memmap', 'memmap'}:
        proc_dir = getattr(args, 'proc_data_path', '') or args.data_path
        res_list = _parse_int_list(getattr(args, 'proc_res_list', '')) or [args.data_load_reso]
        bundle = build_stage2_dataloader(
        ...
        ld_train = bundle.dataloader
        iters_train = len(ld_train)
        vbs = max(1, round(args.batch_size * 1.5))
        ld_val = math.ceil(50000 / vbs)
        print(f'[stage2 dataloader] backend={dataset_backend}, len(dataloader)={iters_train}, len(dataset)={len(bundle.dataset)}, res_list={res_list}')
...
def main_train(args: arg_util.Args):
    ...
    L_mean, L_tail = -1, -1
    epochs_loss_nan = 0
    # build swanlab logger
    if dist.is_master():
        swanlab_utils.initialize(args=args, entity=None, exp_name=args.exp_name, project_name=args.project_name)
