"""ARPC CLI — entropy_{channel/scale/spatial} masking + ZERO header + fixed 1024x1024
...
from codec.arpc_codec_zeroheader_fixed1024_entropy_masks import ARPCNoneZeroHeaderFixed1024
...
def build_codec(args, device: torch.device) -> ARPCNoneZeroHeaderFixed1024:
    ...
    try:
        codec.mask_params = json.loads(getattr(args, "mask_params", "{}") or "{}")
    except Exception:
        codec.mask_params = {}
    # Active bits spec (optional)
...
def main():
    ...
    common.add_argument("--mask_strategy", type=str, default="none",
                        ...
                        help="Masking strategy (0-header: encoder/decoder must match).")
    common.add_argument("--mask_params", type=str, default="{}",
                        help="JSON dict for strategy params, e.g. '{\"keep_ratio\":0.25}'")
    common.add_argument("--active_bits", type=str, default="all",
                        help="Active bits per scale in [1..d]. Use \"all\" or an integer string like \"16\".")
