...
class ARPCZeroHeaderFixed1024:
    ...
    def _precompute_entropy_spatial_global_kept_pos(
        ...
        # Pass-1: MAP-only roll-out to get per-position entropy scores for each scale
        reconstructed_map = []
        Hpos_list = []
        L_list = []
        HW_list = []
        ...
        global_scores = torch.cat(Hpos_list, dim=0)  # [sum L]
        total_L = int(global_scores.numel())

        if entropy_thr is not None:
            thr = float(entropy_thr)
            kept_global = torch.nonzero(global_scores >= thr, as_tuple=False).view(-1).to(torch.long)
            if kept_global.numel() == 0:
                kept_global = _topk_indices(global_scores, k=1)
            kept_global = torch.sort(kept_global)[0]
        else:
        ...
        for si, L in enumerate(L_list):
            ...
            if min_keep_per_scale > 0 and int(sel.numel()) < min_keep_per_scale:
                # pick top positions from this scale and union
                extra = _topk_indices(Hpos_list[si], k=min_keep_per_scale)
    ...
    def compress(self, img_B3HW: torch.Tensor, out_stream_path: str, prompt: str = "") -> bytes:
        ...
        # -------------------------------------------------
        if mask_strategy == "entropy_spatial_global":
    ...
    def decompress(self, stream_path: str, out_path: str, prompt: str = "") -> torch.Tensor:
        ...
        for si in range(int(self.k_transmit)):
            ...
            elif mask_strategy == "entropy_spatial_global":
        ...
        for si in range(int(self.k_transmit)):
            ...
            _, Hs, Ws = scale_schedule[si]
            Hs, Ws = int(Hs), int(Ws)
            if mask_strategy in ("entropy_spatial", "entropy_spatial_global"):
        ...
        if out_path.lower().endswith((".png", ".jpg", ".jpeg", ".bmp", ".webp")):
