...
class ARPCCodec:
    ...
    def encode_image_to_bitidx_and_schedule(self, img_B3HW: torch.Tensor):
        ...
        #  mode=self.vae.quantizer.schedule_mode
        # mode="dynamic"
    ...
    def compress(
        ...
        for si in range(k_transmit):
            ...
            for b, p1v in zip(bits_1d.tolist(), p1_1d.tolist()):
                enc.encode_bit(int(b), float(p1v))
    ...
    def decompress(
        ...
        for si in range(k_transmit):
            ...
            if mask_strategy in ("none", "gm"):
                ...
                for i, p in enumerate(p1_stream.tolist()):
                    bits_stream[i] = dec.decode_bit(float(p))
