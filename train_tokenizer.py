...
def _tensor_to_pil(x_chw: torch.Tensor) -> Image.Image:
    """x_chw in [-1,1], float, shape [3,H,W]"""
...
@torch.no_grad()
def run_val_visualization(d_vae, val_iter_holder, val_loader, device, save_root: str, global_step: int, rank: int,
                          prefix_scales=(2, 3, 4, 5), num_images: int = 4):
    """Periodic validation visualization. Saves num_images PNGs on rank0.

    Each PNG is a horizontal concat: [GT | recon@k2 | recon@k3 | recon@k4 | recon@k5].
    """
    if rank != 0:
        return
    ...
    try:
        ...
        # reset iterator (and sampler epoch if available)
    ...
    # ensure all requested scales exist (if model has fewer scales, clamp)
...
def main():
    ...
    # validation dataloader (used by visu_every)
    ...
    for global_step in range(init_step, args.max_steps):
        ...
        if (global_step+1) % args.log_every == 0:
            ...
            if rank == 0:
                ...
                # writing logs
                logger.info(f'global_step={global_step}, precepetual_loss={avg_loss_dict.get("train/perceptual_loss",0):.4f}, recon_loss={avg_loss_dict.get("train/recon_loss",0):.4f}, bpp_loss={avg_loss_dict.get("train/rate_loss",0):.4f}, dino_loss={avg_loss_dict.get("train/dino_loss",0):.4f}, commitment_loss={avg_loss_dict.get("train/commitment_loss",0):.4f}, logit_r={avg_loss_dict.get("train/logits_image_real",0):.4f}, logit_f={avg_loss_dict.get("train/logits_image_fake",0):.4f}, L_disc={avg_loss_dict.get("train/d_image_loss",0):.4f}, iter_speed={iter_speed:.2f}s')
            start_time = time.time()
