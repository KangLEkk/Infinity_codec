...
class Infinity(nn.Module):
    ...
    def autoregressive_infer_cfg(
        ...
        logits_per_scale = [] if return_logits_per_scale else None
        ...
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            ...
            if vae_type != 0:
                ...
                # Apply forced sparse tokens (for progressive decoding)
                if forced_bitidx_per_scale is not None and forced_mask_per_scale is not None:
                    if si < len(forced_bitidx_per_scale) and forced_bitidx_per_scale[si] is not None:
                        fbits = forced_bitidx_per_scale[si].to(device=idx_Bld.device)
                        fmask = forced_mask_per_scale[si].to(device=idx_Bld.device)
                        # fmask: (B,H,W) or (B,1,H,W) or (B,1,H,W,1) -> (B,1,H,W,1)
                        if fmask.ndim == 3:
                            fmask = fmask.unsqueeze(1).unsqueeze(-1)
                        elif fmask.ndim == 4:
                            fmask = fmask.unsqueeze(-1)
                        fmask = fmask.to(dtype=torch.bool)
                        # fbits: (B,H,W,d) or (B,1,H,W,d) -> (B,1,H,W,d)
                        if fbits.ndim == 4:
                            fbits = fbits.unsqueeze(1)
                        fbits = fbits.to(dtype=idx_Bld.dtype)
                        idx_Bld = torch.where(fmask.expand_as(idx_Bld), fbits, idx_Bld)
        ...
        if not ret_img:
            if return_logits_per_scale:
                return ret, idx_Bl_list, [], logits_per_scale
        ...
        if return_logits_per_scale:
            return ret, idx_Bl_list, img, logits_per_scale
